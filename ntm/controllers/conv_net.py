...
class ConvNetController2(BaseController):
    def __init__(self, num_inputs:int, num_layers:int) -> None:
        super().__init__()
        self.num_inputs = num_inputs
        # self.num_outputs = num_outputs
        ...
        self.model_ = nn.Sequential(
            ...
            ConvBlock(128, 128, (2, 2), padding=0),
            ConvBlock(128, 128, (2, 2), padding=0),
            ConvBlock(128, 256, (2, 2), padding=0),
            ConvBlock(256, 256, (2, 2), padding=0),
            ConvBlock(512, 512, (2, 2), padding=0),


        )
        self.flatten = nn.Flatten()
        self.fc_ = nn.Linear(512, 1024, device=self.device_)
        # model_ = resnet50(pretrained=True)
        # self.feature_extractor = nn.Sequential(*list(model_.children())[:-1]).to(self.device_)
